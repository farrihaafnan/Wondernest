...
const SelectChildForProgress: React.FC = () => {
  ...
  const handleChildSelect = (child: Child) => {
    navigate(`/child-progress/${child.id}`);
  };
  ...
  const handleAddKid = async () => {
    try {
      ...
      const updated = await fetch(`${USER_LEARNING_API_BASE_URL}/api/parents/${user.id}/children`, {
        headers: { Authorization: `Bearer ${token}` }
      }).then(res => res.json());
      setChildren(updated);
      setOpen(false);
  ...
  return (
    <Container>
      <Typography variant="h4" gutterBottom>Select a Child to View Progress</Typography>
...
export default SelectChildForProgress; 